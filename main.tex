...
When trained for 8000 epochs to a final budget of 2000 points, TVAR achieved a substantially lower mean MAE of $\mathbf{0.418 \pm 0.121}$ compared to the industry-standard Residual Adaptive Refinement (RAR) algorithm ($0.564 \pm 0.008$) and the uniform static baseline ($0.585 \pm 0.008$). Strikingly, the high standard deviation of TVAR is driven by a distinctly bimodal distribution: 3 of the 5 initialization seeds successfully escaped early trapping and converged to a markedly superior regime (MAE $\approx 0.31$), while the remaining 2 seeds performed comparably to the RAR baseline (MAE $\approx 0.56$). This behavior perfectly corroborates the metastable-convergence hypothesis detailed previously: if the PINN optimizer falls into a deceptive metastable basin, temporal variance collapses prematurely, degrading the active sampler to static behavior. However, when the optimizer remains dynamic and successfully traverses the energy landscape, Pointwise Temporal Variance provides a vastly superior real-time signal for actively clustering collocation points and accelerating convergence across stiff phase interfaces.
