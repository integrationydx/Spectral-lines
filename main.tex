...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{outputs/double_well.png}
...
To force the optimizer out of metastability and restore the metric's efficacy on simpler PDEs, we introduce an \textit{Active Perturbation Phase}. Instead of passively recording the final 20 epochs, we inject a small, controlled deterministic perturbation ($0.5 \sin(10\pi x)\cos(10\pi t)$) into the physics loss term at epoch $N-20$. If the network has converged to the true physical solution, the sharp gradients of the true basin immediately dampen the perturbation, resulting in low variance. Conversely, if it rests in a shallow, incorrect minimum, the perturbation violently disrupts the state, causing massive variance through forced resonance. Our experiments confirmed this hypothesis: under active perturbation, the false zero-variance state was shattered, and the true correlations emerged (jumping to $r=0.4225$ at Reference 1, and $r=0.5887$ during Interpolation).
\begin{figure}[htbp]
  \centering
